# capstone-data-analytics
Final Capstone Project for Data Analytics Course
A Capstone Data Analytics Project by Brice Kamgang

Overview

This project examines the socioeconomic factors that influence home values across Arizona counties. Using Python, Power BI, and multiple datasets, the analysis explores relationships between:
- Median Home Value
- Household Income
- Poverty Rates
- Education Levels
- Commute Times
- Homeownership Rates
- County Population

Technologies Used

- Python (Pandas, seaborn, Scikit-Learn, Matplotlib)
- Power BI Desktop
- Google Colab
- GitHub for version control

Repository Structure

datasets/           → Raw & processed files
scripts/            → Python processing & modeling
powerbi/            → Final PBIX dashboard
documentation/      → Final Report + Instruction Report
README.md           → Project overview

How to Reproduce the Analysis

1. Clone this repository
2. Open /scripts/final_python.py
3. Place raw dataset files inside /datasets/
4. Run the script
5. Open /powerbi/Final Capstone Report. pbix
6. Connect it to az_housing_merged.csv if prompted

Power BI Dashboard

Includes:
- Overview KPIs
- Income & Poverty Analysis
- Education & Commute Factors
- County Comparisons
- Mobile Layout

Author
Brice Kamgang
Data Analytics & Programming
